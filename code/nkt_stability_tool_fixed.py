...
#
# ---------------------------------------------------------------------------
# FIXED VERSION. Changes relative to the original are marked with "# FIX:".
# Summary of the fixes in this file:
#   1. Wave orbital velocity Uwc1/Uwc2: removed the spurious np.radians() around
#      the sinh() argument (already applied by Willem).
#   2. RoBeD berm coefficient: 6 -> 6000 (already applied by Willem).
#   3. TSH is now the applied Shields number (mobility) on top of the berm for
#      the candidate rock: theta = tau / ((rho_s - rho_w) * g * D50), per the
#      Deltares HaSPro handbook Appendix A. The applied shear tau (TBCW) is now
#      stored during the solve. The old formula had the candidate D50 and the
#      density on the wrong side of the fraction and an extra /1000, which
#      inflated TSH ~1000x and destroyed the berm slope.
#   4. Soil-type test used `== 'Claysandgravel' or 'Sandgravel' or ...`, which is
#      always True. Replaced with a proper membership test.
#   5. The computed Van Rijn threshold D50 never appeared in any output sheet.
#      It is now written to a dedicated "VanRijn threshold" sheet.
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        for i in range(len(df['WL'])):
            ...
            while (in1['D50ic1'][i] < in1['D50c1'][i]):
                ...
                # FIX: no np.radians() on the sinh() argument (it is a dimensionless ratio, not an angle)
            ...
            if in2['D50c2'][i] > in1['D50c1'][i]:
                Hydroberm.at[i, 'D50'] = in2['D50c2'][i]  # FIX: removed /1000 (the value is already in metres)
                Hydroberm.at[i, 'TSh'] = in2['Tetacrshinc2'][i]
                Hydroberm.at[i, 'L'] = in2['LSc2'][i]
                Hydroberm.at[i, 'TBCW'] = in2['TBCWc2'][i]  # FIX: store the governing applied shear for the mobility calc
                Hydroberm.at[i, 'point_ID'] = df['point_ID'][i]
            else:
                Hydroberm.at[i, 'D50'] = in1['D50c1'][i]  # FIX: removed /1000 (the value is already in metres)
                Hydroberm.at[i, 'TSh'] = in1['Tetacrshinc1'][i]
                Hydroberm.at[i, 'L'] = in1['LSc1'][i]
                Hydroberm.at[i, 'TBCW'] = in1['TBCWc1'][i]  # FIX: store the governing applied shear for the mobility calc
                Hydroberm.at[i, 'point_ID'] = df['point_ID'][i]

        # --------------------------------------------------------------------------
        # Rock Berm DHmaxnsion Design
        def calculate_berm_params(i, RD, D50, T0, KP, WHS100, WL, TSh):
            # FIX: TSH is the applied Shields number (mobility) on top of the berm
            # for THIS candidate rock, per Deltares HaSPro Handbook Appendix A:
            #     theta = tau / ((rho_s - rho_w) * g * D50)
            # tau is the combined wave-current bed shear stress (TBCW) computed and
            # stored during the solve above. Note: TBCW was evaluated at the solved
            # threshold rock roughness, so for a candidate of very different size this
            # is a first-order approximation of the shear (good enough for sizing).
            # The old formula had the candidate D50 and the density on the wrong side
            # of the fraction and an extra /1000, inflating TSH ~1000x.
            TSH = Hydroberm['TBCW'][i] / ((RD - Rw) * g * D50)
        ...
        for i in range(len(df['WL'])):
            ...
            # FIX: the original `== 'Claysandgravel' or 'Sandgravel' or ' Sandshell'`
            # is always True (a non-empty string is truthy), so this branch used to
            # fire for every soil type and the `else` below was dead code.
            elif df['ST'][i] in ('Claysandgravel', 'Sandgravel', 'Sandshell'):
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            ...
            # FIX: write the actual Van Rijn threshold results (required D50, critical
            # Shields, applied shear) which were computed but never appeared in any
            # output sheet before. D50 here is the minimum stable rock size in metres.
            Hydroberm.to_excel(writer, sheet_name="VanRijn threshold", index=False)
            # Note: outNR / outHR stay empty for now. Filling them needs an agreed rule
            # for which candidate row is "the chosen design" per point and density.
