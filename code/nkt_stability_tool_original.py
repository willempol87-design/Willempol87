...
gstr = 1 + n * g1
Hydroberm=pd.DataFrame({'point_ID':[], 'D50':[], 'TSh':[],'Bin':[], 'Hberm':[],'Alpha':[],'Wberm':[], 'ABerm':[],'D50Rock':[],'Rrberm':[], 'WD':[],'BBerm':[],'NS':[],'NOD':[]})
Hydroberm['WD'] = Hydroberm['WD'].astype(object)
outNR=pd.DataFrame({'point_ID':[],'SR':[], 'SP':[], 'ST':[], 'WL':[],'DOC':[],'WHS10':[],'Tp10':[],'CU100':[],'WHS100':[],'Tp100':[],'CU10':[], 'D50Threshold(m)':[], 'tetashields':[], 'Hberm(m)':[],'Alpha1:m':[],'Wberm(m)':[], 'D50Rock(m)':[],'Rrberm(kg/m3)':[], 'Hydrodinamic water depth':[],'NOD':[]})
outHR=pd.DataFrame({'point_ID':[],'SR':[], 'SP':[], 'ST':[], 'WL':[],'DOC':[],'WHS10':[],'Tp10':[],'CU100':[],'WHS100':[],'Tp100':[],'CU10':[], 'D50Threshold(m)':[], 'tetashields':[], 'Hberm(m)':[],'Alpha1:m':[],'Wberm(m)':[], 'D50Rock(m)':[],'Rrberm(kg/m3)':[], 'Hydrodinamic water depth':[],'NOD':[]})
...
in2=pd.DataFrame({'D50ic2':[],'D50c2':[],'Ka1c2':[],'Ka2c2':[],'LSc2':[],'Uwc2':[],'fcc2':[],'Awc2':[],'fwc2':[],'TBCWc2':[],'DDc2':[],'Tetacrshic2':[],'D501c2':[],'ksnc2':[],'fwnc2':[],'fcnc2':[],'TBCWnc2':[],'DDnc2':[],'Tetacrshinc2':[],'D50nc2':[]})
...
SlS=pd.DataFrame({'From Point ID:':[ ], 'To Point ID:':[ ],'SRmean':[], 'SPmean':[],'Slope stability':[ ],'Length':[ ]})
    ...
    try:
        ...
        for i in range( len(df['WL'])):
            ...
            while (in1['D50ic1'][i]<in1['D50c1'][i]):
                ...
                in1.at[i,'Ka2c1'] = np.cos(np.radians(df['SP'][i])) * (1 - np.tan(np.radians(df['SP'][i])) ** 2 / np.tan(np.radians(df['FA'][i])) ** 2) ** 0.5
                ...
                in1.at[i,'Uwc1'] = (np.pi * df['WHS10'][i]) / (df['Tp10'][i] * np.sinh(np.radians(2 *np.pi * df['WL'][i] / in1['LSc1'][i])))  #(linear wave theory)
                ...
                in1.at[i,'TBCWc1'] =0.125 * Rw * in1['fcc1'][i] * (gstr * df['CU100'][i]) **2 + 0.25 * in1['fwc1'][i] * Rw * (gstr * in1['Uwc1'][i]) **2
                ...
                in1.at[i,'D50c1'] =  in1['TBCWc1'][i] /((NDR - Rw) * g * in1['Ka1c1'][i] * in1['Ka2c1'][i] * r *in1['Tetacrshic1'][i])
        # calculation of new D50 for check the iterative solution method
                ...
                in1.at[i,'TBCWnc1'] =0.125 * Rw * in1['fcnc1'][i] * (gstr * df['CU100'][i]) **2 + 0.25 * in1['fwnc1'][i] * Rw * (gstr * in1['Uwc1'][i]) **2
                in1.at[i,'DDnc1']=(in1['D501c1'][i] *((NDR/Rw - 1) * g * v **(-2)) **(1 / 3))
                in1.at[i,'Tetacrshinc1'] = 0.3 / (1 +in1['DDnc1'][i] ) + 0.055 * (1 - 1/np.exp(0.02 *in1['DDnc1'][i]))
                in1.at[i,'D50nc1'] = in1['TBCWnc1'][i] /((NDR - Rw) * g *in1['Ka1c1'][i] *in1['Ka2c1'][i] * r * in1['Tetacrshinc1'][i])
            ...
            while (in2['D50ic2'][i]<in2['D50c2'][i]):
                ...
                in2.at[i,'Ka2c2'] = np.cos(np.radians(df['SP'][i])) * (1 - np.tan(np.radians(df['SP'][i])) ** 2 / np.tan(np.radians(df['FA'][i])) ** 2) ** 0.5
                a1 = 2 #based on the rock berm mixture Norsk Stein minimum D50=67.5 and Ks=D90=1.5D50
                in2.at[i,'ksc2'] = a1 * in2['D50ic2'][i] #a11=1.5 to 2
                in2.at[i,'Uwc2'] = (np.pi * df['WHS100'][i]) / (df['Tp100'][i] * np.sinh(np.radians(2 *np.pi * df['WL'][i] / in2['LSc2'][i])))  #(linear wave theory)
                ...
                in2.at[i,'TBCWc2'] =0.125 * Rw * in2['fcc2'][i] * (gstr * df['CU10'][i]) **2 + 0.25 * in2['fwc2'][i] * Rw * (gstr * in2['Uwc2'][i]) **2
                ...
                in2.at[i,'D50c2'] =  in2['TBCWc2'][i] /((NDR - Rw) * g * in2['Ka1c2'][i] * in2['Ka2c2'][i] * r *in2['Tetacrshic2'][i])
         # calculation of new D50 for check the iterative solution method
                ...
                in2.at[i,'TBCWnc2'] =0.125 * Rw * in2['fcnc2'][i] * (gstr * df['CU10'][i]) **2 + 0.25 * in2['fwnc2'][i] * Rw * (gstr * in2['Uwc2'][i]) **2
                in2.at[i,'DDnc2']=in2['D501c2'][i] *((NDR/Rw - 1) * g * v **(-2)) **(1 / 3)
                in2.at[i,'Tetacrshinc2'] = 0.3 / (1 +in2['DDnc2'][i] ) + 0.055 * (1 - 1/np.exp(0.02 *in2['DDnc2'][i]))
                in2.at[i,'D50nc2'] = in2['TBCWnc2'][i] /((NDR - Rw) * g *in2['Ka1c2'][i] *in2['Ka2c2'][i] * r * in2['Tetacrshinc2'][i])
        ...
        for i in range(len(df['WL'])):
                ...
                for D50 in D50_list:
                        ...
                        opt = pd.concat( [opt, calculate_berm_params(i, RD, D50, T0,KP,df['WHS100'][i], df['WL'][i], Hydroberm['TSh'][i])], ignore_index=True)
                        j=j+1
                for D50 in D50_list:
                        RD=HDR
                        #D50_used, HB, NOD = calculate_berm_params(i,RD,  D50, T0)
        ...
        for j in range(len(opt['D50'])):
                ...
                T=6*( opt['TSH'][j])**2+50* opt['TSH'][j]+2.4
...
ctk.CTkLabel(frame_params, text="Dcable (m):" , font=("Times New Roman", 12, "bold")).grid(row=0, column=0, padx=5, pady=5)
entry_Dcable = ctk.CTkEntry(frame_params, width=80)
entry_Dcable.grid(row=0, column=1, padx=5, pady=5)
ctk.CTkLabel(frame_params, text="Berm top width Shallow:" , font=("Times New Roman", 12, "bold")).grid(row=0, column=2, padx=5, pady=5)
entry_TopW= ctk.CTkEntry(frame_params, width=80)
entry_TopW.grid(row=0, column=3, padx=5, pady=5)
ctk.CTkLabel(frame_params, text="Berm top width Deep:" , font=("Times New Roman", 12, "bold")).grid(row=0, column=4, padx=5, pady=5)

entry_TopWD= ctk.CTkEntry(frame_params, width=80)
entry_TopWD.grid(row=0, column=5, padx=5, pady=5)
ctk.CTkLabel(frame_params, text="Normal Rock density(kg/m3):", font=("Times New Roman", 12, "bold")).grid(row=1, column=0, padx=5, pady=5)
entry_NDR = ctk.CTkEntry(frame_params, width=80)
entry_NDR.grid(row=1, column=1, padx=5, pady=5)
ctk.CTkLabel(frame_params, text="High Rock density(kg/m3):", font=("Times New Roman", 12, "bold")).grid(row=1, column=3, padx=5, pady=5)
entry_HDR= ctk.CTkEntry(frame_params, width=80)
entry_HDR.grid(row=1, column=4, padx=5, pady=5)

ctk.CTkLabel(frame_params, text="Maximum Height of Rock Berm(m):", font=("Times New Roman", 12, "bold")).grid(row=2, column=0, padx=5, pady=5)
entry_Hmax = ctk.CTkEntry(frame_params, width=80)
entry_Hmax.grid(row=2, column=1, padx=5, pady=5)
ctk.CTkLabel(frame_params, text="Maximum of Nod:", font=("Times New Roman", 12, "bold")).grid(row=2, column=3, padx=5, pady=5)
...
ctk.CTkButton(app, text="Load Hydroinput details file", font=("Times New Roman", 12, "bold"), command=lambda: load_file(entry_file_path_Hydroinput)).pack(pady=5)
...
ctk.CTkButton(app, text="Load Rock D50 file", font=("Times New Roman", 12, "bold"), command=lambda: load_file(entry_file_path_RockD50)).pack(pady=5)
