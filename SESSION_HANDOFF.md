...
1. **Confirm the 6 changes from section 6e were actually pasted into the live document** (surf zone x2, Ch4.3 directional wave-current paragraph, Herrera & Medina correction, Deltares MOBthresh restore, Ch2.2 closing sentence) — ask for a fresh check-PDF upload, don't assume chat-given text made it in.
2. Investigate `Test.py` and `code/nkt_stability_tool_fixed.py` (see 6e) — figure out where these came from and whether they supersede/relate to the orbital-velocity-bug analysis in section 3 point 4.
