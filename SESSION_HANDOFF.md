...
1. **Confirm the new Ch2.3, new Ch2.4, and new Ch5.9 were actually pasted into the live document** — ask for a re-upload/check PDF like before, don't assume chat-given text made it in. This is now the single most important verification step outstanding.
2. Add the missing DNV-RP-F109 bibliography line to the end of Ch6, and the Thusyanthan bibliography line to the end of Ch5 (both drafted, see above, neither confirmed pasted).

Remaining next steps:
1. Rename/move `cable burring.pdf` → `papers/NKT_2021_CableBurialProtectionPlan.pdf` (or similar) and `DNV-RP-F109...pdf` → `papers/DNV_2021_RP-F109_OnBottomStability.pdf` per the CLAUDE.md convention, and add both (plus the now-moved Thusyanthan paper) to CLAUDE.md's source list (see 6d above) — all are now real, extensively-cited sources, not background reading.
